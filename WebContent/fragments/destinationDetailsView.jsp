...
<form action="FrontController" class="w3-container" method="post">
	...
	<input type="hidden" name="destID" value="${destinationidid }"/>
	...
	<div class="w3-container" id="containerDates">
		...
		<div class="w3-section w3-border">
			<div class="w3-container w3-margin-bottom w3-margin-top">
				...
				jour <input name="depart_jour_${st.index }" value="${dv.dateAller.date }" size="4"/>
				mois <input name="depart_mois_${st.index }" value="${dv.dateAller.month + 1}" size="4"/>
				année <input name="depart_an_${st.index }" value="${dv.dateAller.year + 1900}" size="4"/>
			</div>
			<div class="w3-container w3-margin-bottom">
				<label  class="w3-text-blue">Date de retour : </label>
				jour <input name="retour_jour_${st.index }" value="${dv.dateRetour.date }" size="4"/>
				mois <input name="retour_mois_${st.index }" value="${dv.dateRetour.month + 1}" size="4"/>
				année <input name="retour_an_${st.index }" value="${dv.dateRetour.year + 1900}" size="4"/>
			</div>
			<div class="w3-container w3-margin-bottom">
				...
				<input name="places_${st.index }" value="${dv.nbPlaces }" size="8"/>
			</div>
		</div>
		</c:forEach>
	</div>	
	...
	<div id="toClone" style="display: none">
		<div class="w3-section w3-border">
			<div class="w3-container w3-margin-bottom w3-margin-top">
				<label  class="w3-text-blue">Date de départ : </label>
				jour <input name="depart_jour_" size="4"/>
				mois <input name="depart_mois_" size="4"/>
				année <input name="depart_an_" size="4"/>
			</div>
			<div class="w3-container w3-margin-bottom">
				<label  class="w3-text-blue">Date de retour : </label>
				jour <input name="retour_jour_" size="4"/>
				mois <input name="retour_mois_" size="4"/>
				année <input name="retour_an_" size="4"/>
			</div>
			<div class="w3-container w3-margin-bottom">
				<label  class="w3-text-blue">Tarif : </label>
				<input name="prix_" value="1000" size="8"/>
				<label  class="w3-text-blue">Nb places : </label>
				<input name="places_" value="10" size="8"/>
			</div>
		</div>
	</div>
